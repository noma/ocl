...
int main(int argc, char **argv)
{
	...
	for (size_t i = 0; i < platforms.size(); ++i)
	{
		...
		std::cout << ind << ind << "CL_PLATFORM_NAME:       " << result << std::endl;
		platform.getInfo(CL_PLATFORM_VENDOR, &result);
		std::cout << ind << ind << "CL_PLATFORM_VENDOR:     " << result << std::endl;
		platform.getInfo(CL_PLATFORM_PROFILE, &result);
		std::cout << ind << ind << "CL_PLATFORM_PROFILE:    " << result << std::endl;
		platform.getInfo(CL_PLATFORM_VERSION, &result);
		std::cout << ind << ind << "CL_PLATFORM_VERSION:    " << result << std::endl;
		platform.getInfo(CL_PLATFORM_EXTENSIONS, &result);
		std::cout << ind << ind << "CL_PLATFORM_EXTENSIONS: " << result << std::endl;
